...
func (c *Client) handleTaskQueuedEvent(name string, data def.EDLMTaskQueued) {
	/* If we're dealing with another implant, don't bother. */
	if !c.setiOrNil(data.ID) {
		return
	}
...
func (c *Client) handleTaskRequestEvent(name string, data def.EDLMTaskRequest) {
	...
	/* We didn't have send a task.  We only care if we're in debug mode. */
	if c.Debug {
		sb.WriteString(idn)
		return
	}
...
func (c *Client) handleOutputRequestEvent(name string, data def.EDLMOutputRequest) {
	...
	/* Roll a log message. */
	var sb strings.Builder
	fmt.Fprintf(&sb, "[OUTPUT] From %s", idn) /* Implant name. */
...
func (c *Client) handleListSeenEvent(name string, data def.EDSeen) {
	...
	for _, v := range data {
		...
		s := time.Since(v.When).Round(time.Millisecond)
		if 10*time.Second <= s {
...
func (c *Client) handleFileRequestEvent(name string, data def.EDLMFileRequest) {
	/* If we're dealing with an implant, don't bother. */
	if nil != c.id.Load() {
		return
	}
